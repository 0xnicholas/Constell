...
- Prefer `@constell/shared/src/server` in worker runtime code for queue
...
- Use `@constell/shared` for cross-runtime types, schemas, domain contracts,
  model-pricing helpers, and other frontend-safe utilities.
- Use `@constell/shared/src/db` only when worker code or tests need direct
  Prisma access.
- Use narrower subpaths such as `@constell/shared/src/env` or
  `@constell/shared/encryption` when you specifically need those focused
...
- Be very mindful of adding additional `JSON.parse` calls in the ingestion
  processing pipeline. Those can cause performance issues, because JSONs might
  be very large. Ideally, parse each JSON subset only once.
